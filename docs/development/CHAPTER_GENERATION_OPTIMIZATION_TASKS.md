...
> 历史语义说明（2026-09-17）：下列“PASS 后固定停止并手动提交”记录描述 CGO 实施时的产品决策。GWQ-001 已将当前权威规则更新为小说级 `auto_commit`（默认关闭）；关闭时仍手动确认，开启时安全派发同一个 `CommitChapterJob → CanonicalCommitService`。本节保留用于审计，不再作为当前 Source of Truth。
...
> 当前规则覆盖（2026-09-17）：以下关于 PASS 后固定手动提交的验收记录由 GWQ-001 覆盖；历史完成证据保持不变。当前产品语义以 `docs/PRD.md` 和 `docs/architecture/generation-pipeline.md` 的小说级 `auto_commit` 规则为准。
