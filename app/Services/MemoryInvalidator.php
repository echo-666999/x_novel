...
class MemoryInvalidator
{
    public function invalidateForChapter(int $chapterId): int
    {
        return DB::transaction(function () use ($chapterId): int {
            ...
            $eventIds = StoryEvent::query()
                ->where('novel_id', $chapter->novel_id)
                ->where('chapter_id', $chapter->getKey())
                ->pluck('id');
            $stateVersionIds = StoryStateVersion::query()
                ->where('novel_id', $chapter->novel_id)
                ->where('chapter_id', $chapter->getKey())
                ->pluck('id');
            ...
            return Memory::query()
                ...
                ->where(function (Builder $sources) use ($chapter, $eventIds, $stateVersionIds): void {
                    $sources->where(fn (Builder $source): Builder => $source
                        ->where('source_type', 'chapter')
                        ->where('source_id', $chapter->getKey()));

                    if ($eventIds->isNotEmpty()) {
                        $sources->orWhere(fn (Builder $source): Builder => $source
                    ...
                    if ($stateVersionIds->isNotEmpty()) {
                        $sources->orWhere(fn (Builder $source): Builder => $source
                            ->where('source_type', 'story_state_version')
                            ->whereIn('source_id', $stateVersionIds));
                    }
                })
                ->update(['status' => MemoryStatus::Invalid]);
        });
    }
}
